//! Species identity and sprite rendering. The art itself is in `art.rs`.

mod art;

use crate::pet::Mood;

// Variant order is load-bearing: i18n indexes its per-species tables with
// `species as usize`, and `SPECIES` must list them in the same order.
